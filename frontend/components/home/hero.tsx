...
import { jetBrainsMono, prompt, spaceGrotesk } from "@/fonts";
...
const Hero = ({ profileDetails, isLoading }: AccountProps) => {
  ...
  return (
    ...
    >
        ...
        >
          ...
          {profileDetails ? (
            <span
              ...
              className={`${spaceGrotesk.className} text-center px-3 `}
            >
              Share
            </span>
          ) : (
      ...
      <div className="flex items-center max-sm:flex-col gap-4 mt-10">
        {/* GUEST */}
        <Link
          href={profileDetails ? appUrls.FILE : appUrls.LANG}
        ...
        >
          ...
          >
            {profileDetails ? "Go to Files" : "Continue as Guest"}
            <RxArrowRight size={18} />
          </NRCButton>
        </Link>

        <Link
          href={profileDetails ? appUrls.LANG : appUrls.REGISTER}
          className="rounded-xl"
        >
          <NRCButton
            type="none"
          ...
          >
            {profileDetails ? "Explore Languages" : "Create Account"}
          </NRCButton>
        </Link>
      </div>
